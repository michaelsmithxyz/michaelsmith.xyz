...
const decodeBase64 = (data: string): Uint8Array => new Uint8Array(Buffer.from(data, 'base64'));
...
export const isRawSignedApiKey = (value: unknown): value is RawSignedApiKey =>
  typeof value === 'object' &&
  value !== null &&
  typeof (value as RawSignedApiKey).content === 'string' &&
  typeof (value as RawSignedApiKey).signature === 'string';

export const signApiKey = async (content: Record<string, unknown>): Promise<RawSignedApiKey> => {
  const data = new TextEncoder().encode(JSON.stringify(content));
  const signature = await crypto.subtle.sign(hmacKeyAlgorithm, await getHmacKey(), data);
  return {
...
export const exportApiKey = (key: RawSignedApiKey): string => encodeBase64(JSON.stringify(key));
...
export const isValidSignedApiKey = async (apiKey: RawSignedApiKey): Promise<boolean> =>
